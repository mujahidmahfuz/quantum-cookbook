(function () {
  ...
  function purityOf(rho) { const r2 = matMul2(rho, rho); return r2[0][0].re + r2[1][1].re; }

  const SIZE = 200, CX = 100, CY = 100, R = 80;
  let thetaDeg = 90, gamma = 0.4, channelType = "amplitude";

  function getKraus() {
    const g = gamma;
    if (channelType === "amplitude") {
      return [[[re(1), re(0)], [re(0), re(Math.sqrt(1 - g))]], [[re(0), re(Math.sqrt(g))], [re(0), re(0)]]];
    }
    return [[[re(1), re(0)], [re(0), re(Math.sqrt(1 - g))]], [[re(0), re(0)], [re(0), re(Math.sqrt(g))]]];
  }

  function render() {
    const svg = document.getElementById("damp-svg");
    if (!svg) return;
    ...
    document.getElementById("damp-readout").textContent = `Input: (rx,rz)=(${round2(bIn.rx)}, ${round2(bIn.rz)})    Output: (rx,rz)=(${round2(bOut.rx)}, ${round2(bOut.rz)})    Tr(ρ_out²)=${round2(purityOf(rhoOut))}`;
    document.getElementById("damp-theta-val").textContent = thetaDeg + "°";
    document.getElementById("damp-gamma-val").textContent = round2(gamma);
  }

  function init() {
    if (!document.getElementById("damp-widget")) return;
    document.getElementById("damp-theta-slider").addEventListener("input", (e) => { thetaDeg = parseFloat(e.target.value); render(); });
    document.getElementById("damp-gamma-slider").addEventListener("input", (e) => { gamma = parseFloat(e.target.value); render(); });
    document.querySelectorAll(".damp-type-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        channelType = btn.dataset.type;
        document.querySelectorAll(".damp-type-btn").forEach((b) => b.classList.remove("is-active"));
